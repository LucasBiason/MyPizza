...
def validate_text(text, max_length=0, required=False, verbose=None,
    ...
    if not value:
        if required:
            raise ValidationError(_(f'{verbose} has invalid letters'))
        return value

    s = re.search(r"<[^<]+?>", value)
    if not s:
        if max_length and len(value) > max_length:
            raise ValidationError(_(f'{verbose} more than {max_length} letters'))

        if only_digits:
            if not value.isdigit():
                raise ValidationError(_(f'{verbose} must has only digits.'))

        return value

    raise ValidationError(_(f'{verbose} has letters invalid'))
    ...
    if choice:
        for key, lbl in choice:
            if str(selected) == str(key):
                return selected
        raise ValidationError(_(f'{verbose} is not a valid option'))
...
def validate_integer(value, max_length=10, required=False, verbose=None,
    ...
    if not bool(re.match(_re_integer(max_length), str(value) or '')):
        raise ValidationError(_(f'{verbose} more than {max_length} digits'))

    if len(str(value)) > max_length:
        raise ValidationError(_(f'{verbose} more than {max_length} digits'))

    if minimum and int(value)<minimum:
        raise ValidationError(_(f'{verbose} must be more than {minimum}'))

    if above and int(value)<=above:
        raise ValidationError(_(f'{verbose} must be more than {above}'))

    if maximum and int(value)<maximum:
        raise ValidationError(_(f'{verbose} must be less than {maximum}'))

    if below and int(value)<=below:
        raise ValidationError(_(f'{verbose} must be less than {below}'))
...
def validate_decimal(value, max_digits=10, decimal_places=2, required=False, verbose=None,
    ...
    if len(value.split('.')[0]) > max_digits:
        raise ValidationError(_(f'{verbose} more than {max_digits} integer digits'))

    if len(value.split('.')[1]) > decimal_places:
        raise ValidationError(_(f'{verbose} more than {decimal_places} decimal digits'))
